...
<div class="sticky top-[10rem] space-y-6">
    ...
    <div class="bg-slate-800 border border-slate-700 p-4 rounded">
        ...
        <form action="<?php echo esc_url( get_post_type_archive_link( 'sgu_apod' ) ); ?>" method="get">
            ...
            <div class="flex gap-2">
                <input type="search" name="s" placeholder="Search..." class="flex-1 rounded px-3 py-2 bg-slate-900 border border-slate-700 text-slate-200 text-sm">
                ...
                <a href="<?php echo esc_url( get_post_type_archive_link( 'sgu_apod' ) ); ?>" class="px-3 rounded py-2 bg-slate-700 text-slate-200 hover:bg-slate-600 transition-colors" title="Reset">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-slate-800 border border-slate-700 p-4 rounded">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="<?php echo esc_url( get_post_type_archive_link( 'sgu_apod' ) ); ?>" class="px-2 rounded py-1 bg-slate-900 border border-slate-700 text-slate-300 text-sm hover:bg-cyan-600 hover:text-white">RESET</a>
            <?php foreach( $years as $y ) : ?>
            <a href="<?php echo esc_url( add_query_arg( 'apod_year', $y, get_post_type_archive_link( 'sgu_apod' ) ) ); ?>" class="px-2 rounded py-1 bg-slate-900 border border-slate-700 text-slate-300 text-sm hover:bg-cyan-600 hover:text-white"><?php echo $y; ?></a>
            <?php endforeach; ?>
        </div>
    </div>
